...
Trigger examples:
- `Generate in-depth notes for this paper`
- `Write a high-quality paper and take intensive reading notes`
- `Organize this article into obsidian Notes`
...
- say `Note completed` only when the required workflow is actually complete
- say `Draft generated` when drafting is done but lint, final readability review, or save is still pending
- say `Passed verification` only when lint has actually been run and passed
- say `saved to Obsidian` only when the write step has actually succeeded
- do not treat `lint Passed` as equivalent to `The entire note has been polished`
...
- model-first: scripts structure evidence, but the model must decide emphasis, contribution, mechanism, limitations, and final English prose
- required structure: include the canonical required sections, with `Abstract Translation` before `One-Sentence Summary` and a dedicated `Key Innovations` section immediately after `Abstract Translation`
- English output: write the note body in natural academic English; quoted source text may remain in its original language
- abstract rendering: when abstract metadata exists, `Abstract Translation` is a faithful English rendering of the original abstract, not a bilingual block and not the model's own summary
- mechanism depth: method, framework, and system papers should include `### Mechanism Flow` under `Method Overview`, normally as a 3 to 4 step numbered flow with input, operation, and output destination
...
- `## Core Info` must be a fixed metadata block only. Use only these fields, in this order, as `- Field name: value` bullets: `Title`, `Title Translation`, `Authors`, `Affiliations`, `Published`, `Venue`, `DOI`, `arXiv`, `Paper Link`, `Code / Project`, `Data / Resources`, `Paper Type`. Omit unavailable fields; put any guide sentence, takeaway, or analysis in `One-Sentence Summary` or a later section instead.
- The note should include `Abstract Translation` near the beginning when abstract metadata is available, before `One-Sentence Summary`.
- When abstract metadata is available, `Abstract Translation` should faithfully render the original paper abstract in English rather than restating it as your own summary. If the source abstract is already English, preserve its meaning in polished English without adding analysis.
- The `Abstract Translation` section should contain English prose only; quoted source text elsewhere may remain in its original language.
- Do not mix later judgments, innovation summaries, or hindsight explanations into `Abstract Translation`; keep it faithful to the original abstract.
- The note should include a dedicated `Key Innovations` section immediately after `Abstract Translation` and before `One-Sentence Summary`.
- The `Key Innovations` section should not be empty praise. It should enumerate the paper's actual innovations and briefly explain why each one matters.
...
- The note must pass a style gate: body prose must be English; quoted source text and citation metadata may remain in the original language.
- The style gate also rejects mechanical term-replacement artifacts such as `KVcache of`, `batching`, `InRelated Researcher`, or `Single sequence generation`; rewrite the sentence naturally instead of preserving a partially translated phrase.
...
- in `final_readability_review`, rewrite awkward or fragmentary phrasing into natural academic English, while preserving stable proper nouns and source-faithful terminology
